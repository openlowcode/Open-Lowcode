...
/**
 * A substraction of two values on the same object. As any formula element, it
 * should be robust to null values (substraction of two values if one is null
 * returns null also).
 * 
 ...
 */
public class SameObjectSubstract<E extends DataObject<E>> implements FormulaElement<E> {
	private static Logger logger = Logger.getLogger(SameObjectSubstract.class.getName());
	...
	public BigDecimal getValueForFormulaInput(E contextobject) {
		if ((element2.getValueForFormulaInput(contextobject) != null)
			...
			BigDecimal result = element1.getValueForFormulaInput(contextobject)
					.subtract(element2.getValueForFormulaInput(contextobject));
			logger.fine(" Summing two elements of same object " + contextobject.getName() + " result = " + result);
			return result;
		} else {
			logger.fine(" Substraction is null as one of the elements is null for " + contextobject.getName());
			return null;
		}
