 ...
 */
public class ScheduleStartTimeFlatFileLoader<E extends DataObject<E> & UniqueidentifiedInterface<E> & TimeslotInterface<E> & ScheduleInterface<E, F>, F extends DataObject<F> & UniqueidentifiedInterface<F> & ScheduledependencyInterface<F, E> & AutolinkobjectInterface<F, E>>
	 ...
	 */
	public ScheduleStartTimeFlatFileLoader(DataObjectDefinition<E> dataobjectdefinition,
			ScheduleDefinition<E, F> scheduledefinition, String dateformat, PropertyExtractor<E> propertyextractor) {
		this.dataobjectdefinition = dataobjectdefinition;
		this.scheduledefinition = scheduledefinition;
		this.propertyextractor = propertyextractor;
		this.dateformat = dateformat;
		format = FlatFileLoader.generateFormat(dateformat,
				"Schedule Start Time Loader for object " + dataobjectdefinition.getName());
	}

	@Override
	public boolean load(E object, Object value, PostUpdateProcessingStore<E> postupdateprocessingstore) {
		Date date = FlatFileLoader.parseDate(value, "Parsing Schedule Start Tile for object "
				+ dataobjectdefinition.getName() + " for format = " + dateformat, true, format);
		...
		if (!(property instanceof Schedule))
		...
		if (parenttimeslot.getStarttime() == null)
			parenttimeslot.SetStarttime(date);
