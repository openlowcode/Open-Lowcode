 ...
 * numbers. In the flat file, a series of numbers separated by '|' are entered
 * in the flat file, and necessary links are created. If the corresponding
 * option is set, missing links are also deleted
 * 
 ...
 */
public class LeftforlinkFlatFileLoader<E extends DataObject<E> & NumberedInterface<E> & UniqueidentifiedInterface<E>, F extends DataObject<F> & LinkobjectInterface<F, E, G>, G extends DataObject<G> & NumberedInterface<G> & UniqueidentifiedInterface<G>>
	...
	private LinkobjectDefinition<F, E, G> linkobjectpropertydef;
	...
	private ConstraintOnLinkObject<E, G>[] constraints;
	 ...
	 *                                  link)
	 * @param linkobjectdefinition      definiton of the data object that holds the
	 *                                  link
	 * @param rightobjectdefinition     definition of the right data object
	 *                                  definition that holds the link
	 * @param linkobjectpropertydef     definition of the link object property
	 ...
	 */
	public LeftforlinkFlatFileLoader(DataObjectDefinition<E> leftobjectdefinition,
			DataObjectDefinition<F> linkobjectdefinition, DataObjectDefinition<G> rightobjectdefinition,
			LinkobjectDefinition<F, E, G> linkobjectpropertydef, NumberedDefinition<G> rightobjectnumberproperty,
			boolean createrightifnotexists, boolean deletemissinglink, String hardcodedvalue,
		...
		this.linkobjectpropertydef = linkobjectpropertydef;
	...
	public boolean isStaticPreProcessing() {
		...
		if (linkobjectpropertydef.getConstraintOnLinkObjectNumber() > 0)
			return false;
		return true;
	}
		...
		if (next.length() > 0) {
			...
			for (int i = 0; i < rightobjectnumbers.length; i++) {
				...
				if (!rightobjectmapbynumber.containsKey(thisrightobjectnumber)) {
					...
					if (thisrightobject.length > 1)
						throw new RuntimeException("found several right objects " + rightobjectdefinition.getName()
	...
	public boolean load(E object, Object value, PostUpdateProcessingStore<E> postupdateprocessingstore) {
		...
		HashMap<DataObjectId<G>, F> oldlinksfromrightid = new HashMap<DataObjectId<G>, F>();
		F[] existinglinks = LinkobjectQueryHelper.get().getalllinksfromleftid(leftid, null, linkobjectdefinition,
				leftobjectdefinition, rightobjectdefinition, linkobjectpropertydef);
		for (int i = 0; i < existinglinks.length; i++)
			oldlinksfromrightid.put(existinglinks[i].getRgid(), existinglinks[i]);
		HashMap<DataObjectId<G>, F> newlinksfromrightid = new HashMap<DataObjectId<G>, F>();
			...
			for (int i = 0; i < rightobjectnumbers.length; i++) {
				...
				if (leftid != null) {
					...
					if (linkobjectpropertydef.getConstraintOnLinkObjectNumber() == 0) {
						...
						for (int k = 0; k < linkobjectpropertydef.getConstraintOnLinkObjectNumber(); k++) {
							ConstraintOnLinkObject<E, G> constraintonlinkobject = linkobjectpropertydef
									.getConstraintOnLinkObject(k);
					...
					if (rightobject == null)
						if (this.createrightifnotexists) {
							G newrightobject = rightobjectdefinition.generateBlank();
							newrightobject.setobjectnumber(thisrightobjectnumber);
							for (int j = 0; j < linkobjectpropertydef.getConstraintOnLinkObjectNumber(); j++) {
								ConstraintOnLinkObject<E, G> constraint = linkobjectpropertydef
					...
					DataObjectId<G> rightobjectid = rightobject.getId();

					F existinglink = oldlinksfromrightid.get(rightobjectid);
					if (existinglink != null)
						newlinksfromrightid.put(rightobjectid, existinglink);
					...
					if (existinglink == null) {
						...
						link.insert();
						newlinksfromrightid.put(rightobjectid, link);
						oneupdate = true;
		...
		if (this.deletemissinglink) {
			...
			while (oldlinksiterator.hasNext()) {
				...
				if (newlinksfromrightid.get(thisoldlink.getRgid()) == null)
					thisoldlink.delete();
			}
		}
	...
	protected boolean putContentInCell(E currentobject, Cell cell, String context) {
		...
		TwoDataObjects<F, G>[] links = LinkobjectQueryHelper.get().getlinksandrightobject(currentobject.getId(), null,
				linkobjectdefinition, leftobjectdefinition, rightobjectdefinition, linkobjectpropertydef);
