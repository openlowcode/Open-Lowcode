 ...
 * @param <E>
 */
public class TimeslotEndTimeFlatFileLoader<E extends DataObject<E> & TimeslotInterface<E>>
	 ...
	 */
	public TimeslotEndTimeFlatFileLoader(DataObjectDefinition<E> dataobjectdefinition,
		...
		this.format = FlatFileLoader.generateFormat(dateformat,
				"Timeslot EndTime for object " + dataobjectdefinition.getName());
	...
	public boolean load(E object, Object value, PostUpdateProcessingStore<E> postupdateprocessingstore) {
		Date date = FlatFileLoader.parseDate(value,
				"Timeslot EndTime for object " + dataobjectdefinition.getName() + " for format " + dateformat, true,
				format);

		DataObjectProperty<E> property = propertyextractor.extract(object);
		if (property == null)
			throw new RuntimeException("Technical error in inserting end time: property not found");
		if (!(property instanceof Timeslot))
			throw new RuntimeException("Technical error in inserting end time: property not of correct class: "
					+ property.getClass().getName());
		Timeslot<E> timeslot = (Timeslot<E>) property;
		Date olddate = timeslot.getEndtime();
		if (FlatFileLoader.isTheSame(olddate, date)) {
			return false;
		} else {
			timeslot.SetEndtime(date);
	...
	protected boolean putContentInCell(E currentobject, Cell cell, String context) {
		...
		if (!(property instanceof Timeslot))
			throw new RuntimeException("Technical error in inserting start date: property not of correct class: "
					+ property.getClass().getName());
		Timeslot<E> timeslot = (Timeslot<E>) property;
		cell.setCellValue(timeslot.getEndtime());
		return false;
