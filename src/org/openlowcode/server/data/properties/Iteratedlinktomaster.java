...
/**
 * The property specifying that a link to master is iterated. This is the case when the
 ...
 */
public class Iteratedlinktomaster<E extends DataObject<E> & LinkobjecttomasterInterface<E, F, G> & IteratedlinktomasterInterface<E, F, G>, F extends DataObject<F> & IteratedInterface<F>, G extends DataObject<G> & VersionedInterface<G>>
		extends DataObjectProperty<E> {
	private StoredField<Integer> lffirstiter;
	private StoredField<Integer> lflastiter;
	private Linkobjecttomaster<E, F, G> linkobject;
	private IteratedlinktomasterDefinition<E, F, G> casteddefinition;
	...
	public Iteratedlinktomaster(IteratedlinktomasterDefinition<E, F, G> definition, DataObjectPayload parentpayload) {
	...
	public String generateUpdateNote(String action) {
		...
		if ((this.casteddefinition.getRightobjectdef().hasProperty("NUMBERED"))
			...
			DataObjectMasterId<G> rightobjectmasterid = linkobject.getRgmsid();
			G rightobject = rightobjectmasterid.lookupObject();
			if (rightobject == null) {
	 ...
	 */
	public static <E extends DataObject<E> & LinkobjecttomasterInterface<E, F, G> & IteratedlinktomasterInterface<E, F, G>, F extends DataObject<F> & IteratedInterface<F>, G extends DataObject<G> & VersionedInterface<G>>

			String[] generateMassiveUpdateNote(Iteratedlinktomaster<E, F, G>[] preprociteratedlinkbatch, E[] objectbatch,
					String action) {
		...
		if ((preprociteratedlinkbatch[0].casteddefinition.getRightobjectdef().hasProperty("NUMBERED"))
			...
			ArrayList<DataObjectMasterId<G>> rightobjectids = new ArrayList<DataObjectMasterId<G>>();
			for (int i = 0; i < objectbatch.length; i++) {
				rightobjectids.add(preprociteratedlinkbatch[i].linkobject.getRgmsid());
			}
			DataObjectDefinition<G> rightobjectdefinition = preprociteratedlinkbatch[0].casteddefinition
					.getRightobjectdef();
			DataObjectMasterId<G>[] rightobjectidarray = rightobjectids
					.toArray(rightobjectdefinition.generateMasterIdArrayTemplate());

			G[] rightobjects = VersionedQueryHelper.get().getlastversion(rightobjectidarray, preprociteratedlinkbatch[0].casteddefinition.getRightobjectdef(), preprociteratedlinkbatch[0].casteddefinition.getRightObjectVersioned());
	 ...
	 */
	public void setDependentPropertyLinkobjecttomaster(Linkobjecttomaster<E, F, G> linkobject) {
	 ...
	 *                          object provided
	 */
	public static <E extends DataObject<E> & LinkobjecttomasterInterface<E, F, G> & IteratedlinktomasterInterface<E, F, G>, F extends DataObject<F> & IteratedInterface<F>, G extends DataObject<G> & VersionedInterface<G>> void archivethisiteration(
			E[] objectbatch, Iteratedlinktomaster<E, F, G>[] iteratedlinkbatch, Integer[] leftobjectolditer) {
		for (int i = 0; i < objectbatch.length; i++) {
	 ...
	 */
	public static <E extends DataObject<E> & LinkobjecttomasterInterface<E, F, G> & IteratedlinktomasterInterface<E, F, G>, F extends DataObject<F> & IteratedInterface<F>, G extends DataObject<G> & VersionedInterface<G>> void preprocUniqueidentifiedUpdate(
			DataObjectDefinition<E> definition, E[] objectbatch, Iteratedlinktomaster<E, F, G>[] preprociteratedlinkbatch) {
		...
		for (int i = 0; i < oldobjects.length; i++) {
			Iteratedlinktomaster<E, F, G> iteratedlinkforold = oldobjects[i].getPropertyForObject(preprociteratedlinkbatch[0]);
		...
		if (leftobjectdefinition.hasProperty("ITERATED")) {
			String[] updatenotes = Iteratedlinktomaster.generateMassiveUpdateNote(preprociteratedlinkbatch, objectbatch,
					"Update");
		...
		for (int i = 0; i < leftobjects.length; i++) {
			leftobjectui[i] = leftobjects[i].getUniqueidentiedFromLinkObjectToMaster(preprociteratedlinkbatch[0].linkobject);
		}
	 ...
	 *                                 properties
	 */
	public static <E extends DataObject<E> & LinkobjecttomasterInterface<E, F, G> & IteratedlinktomasterInterface<E, F, G>, F extends DataObject<F> & IteratedInterface<F>, G extends DataObject<G> & VersionedInterface<G>> void preprocUniqueidentifiedDelete(
			DataObjectDefinition<E> definition, E[] objectbatch, Iteratedlinktomaster<E, F, G>[] preprociteratedlinkbatch) {
		ArrayList<DataObjectId<F>> leftobjectids = new ArrayList<DataObjectId<F>>();
		for (int i = 0; i < objectbatch.length; i++) {
			leftobjectids.add(preprociteratedlinkbatch[i].linkobject.getLfid());
		...
		if (hasobjects) {
			...
			DataObjectPayload[] oldpayloads = new DataObjectPayload[oldobjects.length];
			for (int i = 0; i < oldobjects.length; i++) {
				Iteratedlinktomaster<E, F, G> iteratedlinkforold = oldobjects[i]
			...
			for (int i = 0; i < leftobjects.length; i++) {
				leftobjectui[i] = leftobjects[i]
						.getUniqueidentiedFromLinkObjectToMaster(preprociteratedlinkbatch[0].linkobject);
			}
			// 3 - update left object (with id)
			Uniqueidentified.update(leftobjects, leftobjectui);
			for (int i = 0; i < objectbatch.length; i++) {
	 ...
	 */
	public static <E extends DataObject<E> & LinkobjecttomasterInterface<E, F, G> & IteratedlinktomasterInterface<E, F, G>, F extends DataObject<F> & IteratedInterface<F>, G extends DataObject<G> & VersionedInterface<G>> void preprocStoredobjectInsert(
			E[] objectbatch, Iteratedlinktomaster<E, F, G>[] preprociteratedlinkbatch) {
