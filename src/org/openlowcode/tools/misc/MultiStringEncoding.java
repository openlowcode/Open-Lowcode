 ...
 * values in a simple and compact way. Encoding principles:
 * <ul>
 * <li>basic separator is '|' (pipe)</li>
 * <li>if a value includes '|', an error is thrown</li>
 * </ul>
 * As stated above, this simple algorithm will not work for null values inside
 * the array (else, you should use CSV instead)
 * 
...
public class MultiStringEncoding {
	...
	public static String[] parse(String encodedmultistring) {
		if (encodedmultistring == null)
			return new String[0];
		if (encodedmultistring.length() == 0)
			return new String[0];
	...
	public static String encode(String[] multistringtoencode) {
		if (multistringtoencode == null)
			return "";
		if (multistringtoencode.length == 0)
			return "";
		StringBuffer result = new StringBuffer();
		for (int i = 0; i < multistringtoencode.length; i++) {
			String value = multistringtoencode[i];
			if (value.indexOf('|') != -1)
				throw new RuntimeException("Cannot encode a String with a '|' character inside");
			if (i > 0)
				result.append('|');
			result.append(value);
		}
	...
	/**
	 * @param multistringtoencode a list of string
	 * @return a single string
	 */
	public static String encode(List<String> multistringtoencode) {
		if (multistringtoencode == null)
			return "";
		if (multistringtoencode.size() == 0)
			return "";
		StringBuffer result = new StringBuffer();
