...
public class ChoiceDataElt<E extends Choice> extends SimpleDataElt {
	...
	public String[] getAuthorizedValues() {
		if (storedauthorizedvalues==null) return null;
	...
	private ChoiceDataElt(String name,String storedvalue,boolean restriction,ArrayList<String> storedauthorizedvalues) {
	...
	/**
	 * generates a choice data element with no restriction on the values of the field
		...
		if (!value.isTransitionrestrictions()) {
			this.restriction = false;
				...
				for (int i=0;i<value.getAuthorizedTransitions().length;i++) {
					logger.finest("!!**!! adding transition "+i+" -- "+value.getAuthorizedTransitions()[i].getStorageCode());
	...
	protected MessageFieldSpec getMessageFieldSpec() {
		if (restriction) throw new RuntimeException("Restricted Choice Value not supported by compact array");
	...
	public String defaultTextRepresentation() {
		// TODO problem: this is never filled
	...
	public ChoiceDataElt<E> cloneElt()  {
		...
		if (this.storedauthorizedvalues!=null) for (int i=0;i<this.storedauthorizedvalues.size();i++) {
	...
	public boolean defineRestriction(ArrayList<String> constraint)  {
		...
		if (storedauthorizedvalues==null) return false;
		if (storedauthorizedvalues.size()==0) return false;
		boolean valid = false;
		for (int i=0;i<constraint.size();i++) if (this.storedvalue!=null) if (this.storedvalue.equals(constraint.get(i))) valid = true;
	...
	public boolean equals(Object other) {
		...
		if (!(other instanceof ChoiceDataElt)) return false;
		@SuppressWarnings("rawtypes")
		ChoiceDataElt choicedataelt = (ChoiceDataElt) other;
		if (this.storedvalue==null) return false;
